...
from app.mam.providers.gemini import (
    GeminiProvider,
...
def text_response(
    text: str, finish_reason: types.FinishReason = types.FinishReason.STOP
...
def test_constructor_requires_all_fields():
    with pytest.raises(ValueError):
        GeminiProvider(
            project_id="", location="global", model_flash="gemini-3.7-flash", model_pro="gemini-3.1-pro-preview"
        )
    with pytest.raises(ValueError):
        GeminiProvider(
            project_id="darwesh-group",
...
async def test_normal_completion_returns_text():
    ...
    provider._client.aio.models.generate_content = AsyncMock(
        return_value=text_response("Here are 2 real listings in Erbil.")
    )

    result = await provider.generate(
        system_instruction="You are MAM.",
        history=[ChatTurn(role="user", content="villa in Erbil")],
        tools=[],
        tier=ModelTier.FAST,
...
async def test_reasoning_tier_uses_pro_model():
    ...
    await provider.generate(
        system_instruction="x", history=[], tools=[], tier=ModelTier.REASONING, max_output_tokens=800
    )
    _, kwargs = provider._client.aio.models.generate_content.call_args
...
async def test_tool_call_without_id_gets_a_synthetic_call_id():
    ...
    result = await provider.generate(
        system_instruction="x", history=[], tools=[], tier=ModelTier.FAST, max_output_tokens=800
    )
...
async def test_empty_candidates_raises():
    provider = make_provider()
    provider._client.aio.models.generate_content = AsyncMock(
        return_value=types.GenerateContentResponse(candidates=[])
    )
...
async def test_candidate_with_no_parts_raises_empty_response():
    ...
    response = types.GenerateContentResponse(
        candidates=[
            types.Candidate(content=types.Content(role="model", parts=[]), finish_reason=types.FinishReason.SAFETY)
        ]
    )
    provider._client.aio.models.generate_content = AsyncMock(return_value=response)
...
async def test_api_error_is_converted_to_safe_runtime_error():
    ...
    provider._client.aio.models.generate_content = AsyncMock(
        side_effect=APIError(
            code=503,
            response_json={"error": {"message": "internal quota detail nobody outside Google should see"}},
        )
    )
...
def test_history_to_contents_tool_role_becomes_function_response():
    turn = ChatTurn(
        role="tool", content='{"count": 1, "results": []}', tool_name="search_properties", tool_call_id="c1"
    )
    contents = _history_to_contents([turn])
