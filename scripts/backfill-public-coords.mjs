...
// NOT close the exposure. The rules stop any NEW lat/lng from being written
// to a listing document, but every listing created before that still carries
// its real lat/lng on the same document any anonymous visitor can read
// (`listings` is `allow read: if true`). THIS SCRIPT is what removes them.
//
// For each listing it:
...
//
// Idempotent: rounding is deterministic, and a listing already migrated (no
// lat/lng, has publicLat/publicLng) is skipped. Safe to re-run after a
// partial failure.
//
// Runs with the Admin SDK, which bypasses security rules — required here,
// since the rules deliberately forbid the very lat/lng-carrying documents
// this script is cleaning up.
//
...
// Credentials. Against the emulator (FIRESTORE_EMULATOR_HOST set) the Admin
// SDK needs none at all. Against production it uses Application Default
// Credentials, which resolve in this order:
//
//   1. GOOGLE_APPLICATION_CREDENTIALS -> a service-account key file, if you
//      happen to have one (NOT required),
...
//
// (2) is the intended way to run this. A downloaded service-account key is a
// long-lived credential that is easy to leak and awkward to rotate;
// requiring one for a migration a human runs by hand would be a worse
// security posture than the exposure being fixed.
//
// Nothing about the credential is ever printed — only the project id, which
// is not a secret.
...
// How far the rounded public point sits from the real one -- reported in the
// dry run so the precision loss is a visible number, not an assumption.
...
async function main() {
  ...
  for (const docSnap of snap.docs) {
    ...
    const hasPrecise = 'lat' in d || 'lng' in d;
    ...
    if (pub.publicLat === undefined) {
      // lat/lng present but not a usable pair (null, '', false, a
      // non-numeric string, or only one half of the pair). Strip them anyway
      // rather than leaving a precise-looking value on a public document,
      // but report it so someone can re-pin the listing. Never invent a
      // public point from a half pair.
    ...
    if (APPLY) {
      // Exact pair preserved FIRST, in the same batch as the deletion, so
      // there is no committed state where the exact coordinate has been
      // removed from the listing but not yet recorded in private/location.
    ...
    plan.push({
      ...
      shiftMetres: Math.round(haversineMetres(Number(d.lat), Number(d.lng), pub.publicLat, pub.publicLng)),
    });
  ...
  if (!APPLY && plan.length) {
    ...
    console.log('  listing id                    exact lat/lng    ->  public pair        moved');
